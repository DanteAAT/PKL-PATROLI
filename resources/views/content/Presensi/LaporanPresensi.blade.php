...
@section('content')
    ...
    <h3 class="page-title">
        <b>Laporan Presensi</b> <small>Mengelola Presensi</small>
    </h3>
    <br />
    <div id="accordion">
        <form method="post" action="{{ route('filter-laporan-presensi') }}" enctype="multipart/form-data">
            @csrf
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="selection-search-clear select-form" aria-label="Default select example"
                                    name="personil_id">
                                    <option value="" hidden></option>
                                    @foreach ($personil as $personils)
                                        @if ($personils['personil_id'] == $personil_id)
                                            <option value="{{ $personils['personil_id'] }}" selected>{{ $personils['name'] }}</option>
                                        @else
                                            <option value="{{ $personils['personil_id'] }}">{{ $personils['name'] }}</option>
                                        @endif
                                    @endforeach
                                </select>
                            </div>
    ...
    <div class="card border border-dark">
        <div class="card-header bg-dark clearfix">
            ...
            <div class="form-actions float-right"> 
                <button onclick="location.href='{{ url('/laporan-presensi/cetak-pdf') }}'" class="btn btn-sm btn-info" title="Add Data"
                    style="margin-left: 5px"><i class="fa fa-print"></i> Print PDF
                </button>
                <button onclick="location.href='{{ url('/laporan-presensi/cetak-excel') }}'" class="btn btn-sm btn-info"
                    ...
                    <tbody>
                        ...
                        @foreach ($presensi as $presensis)
                            <tr>
                                ...
                                <td width="15%">{{ $presensis->callPersonil['name'] }}</td>
                                <td width="15%" style="text-align:center">{{ $presensis->callPatrol['patrol_name'] }}</td>
                                <td width="15%" style="text-align:center">{{ $presensis->callLocation['location_name'] }}</td>
                                <td width="15%" style="text-align:center">
                                    {{ date('d-m-Y', strtotime($presensis['date_time'])) }}</td>
                                <td width="15%" style="text-align:center">
                                    {{ date('H:i:s', strtotime($presensis['date_time'])) }}</td>
                                <td width="20%" style="text-align:center">{{ $presensis['information'] }}</td>
                            </tr>
                            <?php $no++; ?>
                        @endforeach
                    </tbody>
